		...
		if (si.getOrderItems() != null && !si.getOrderItems().isEmpty()) {
			for (SalesInvoiceItem item : si.getOrderItems()) {
				if (!itemRepo.existsById(item.getItem().getId())) {
					...
					Date expiryDate = invRepo.findExpiryDateIfExists(si.getBranchLocation().getId(), item.getItem().getId());
					if (expiryDate != null && expiryDate.compareTo(new java.sql.Date(si.getInvoiceDate().getTime())) < 0) {
