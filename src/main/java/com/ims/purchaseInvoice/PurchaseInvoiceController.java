		...
		if (pi.getOrderItems() != null && !pi.getOrderItems().isEmpty()) {
			for (PurchaseInvoiceItem item : pi.getOrderItems()) {
				if (!itemRepo.existsById(item.getItem().getId())) {
					...
					Date expiryDate = invRepo.findExpiryDateIfExists(pi.getBranchLocation().getId(), item.getItem().getId());
					if (expiryDate != null && expiryDate.compareTo(new java.sql.Date(pi.getInvoiceDate().getTime())) < 0) {
